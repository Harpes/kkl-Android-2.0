...
from nonebot import CommandSession, Message, on_command
...
async def group_ban(context):
    ...
    try:
        ...
        f_manager = [i['user_id'] for i in group_memberinfo if i['role']
                     == 'owner' or i['role'] == 'admin']
...
async def unset_ban(session: CommandSession):
    ...
    f_manager = [i['user_id'] for i in group_memberinfo if i['role']
                 == 'owner' or i['role'] == 'admin']
    # 成员小于100人
    if session.ctx['message_type'] == 'group' and session.ctx['user_id'] in f_manager and len(group_memberinfo) <= 100:
