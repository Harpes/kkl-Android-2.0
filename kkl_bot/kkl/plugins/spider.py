...
new_switch, hbook_switch, jjcsearch_switch = True, False, True
...
@on_command('Tnews', aliases=('台服新闻', '台服公告'), only_to_me=False)
...
@on_command('Tevents', aliases=('台服活动',), only_to_me=False)
async def Tevents(session: CommandSession):
    ...
    for i in data:
        ...
        if t > n:
            msg = ('\n-----------------------------------------------\n活动名称：{}\n活动时间：{}--{}'.format(
                jianfan.toSimpleString(i['campaign_name']), i['start_time'][5:], i['end_time'][5:]))
            msg0 += msg
    await session.send(message=msg0)
        ...
        if r:
            ...
            for i in range(len(data['docs'])):
                if data['docs'][i]['title_chinese'] in d.keys():
                    ...
                    d[jianfan.toSimpleString(data['docs'][i]['title_chinese'])] = [
